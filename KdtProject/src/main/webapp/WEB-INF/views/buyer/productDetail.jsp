			...
			<tbody>
				<c:if test="${not empty options}">
							...
							<tbody>
								<tr class="text-center">
									...
									<td class="border px-4 py-2">
										<select name="productSize" id="sizeSelect" class="border rounded px-2 py-1" required>
									...
									<td class="border px-4 py-2">
										<c:choose>
											<c:when test="${not empty firstOption.product.productPhoto}">
												<img src="${imagekitUrl}product/${firstOption.product.productPhoto}"
											...
											<tr class="text-center">
												...
												<td class="border px-4 py-2">
													...
													<c:if test="${review.userId eq sessionScope.loginUser.id}">
														<div class="mt-2">
															<form action="${pageContext.request.contextPath}/mypage/product/review/delete"
																method="post" onsubmit="return confirm('리뷰를 삭제하시겠습니까?');" style="display: inline;">
															...
															<%-- 리뷰 수정 버튼 --%>
															<button onclick="showEditForm('${review.reviewId}', '${review.score}', 
					...
					<div id="editReviewForm" class="mt-8 hidden">
						...
						<form action="${pageContext.request.contextPath}/mypage/product/review/update"
							...
							<div class="mb-4">
								...
								<select name="score" id="editScore" required class="border rounded px-3 py-2 w-full">
					...
					<c:if test="${not alreadyReviewed and not empty sessionScope.loginUser}">
						<div class="mt-8">
							...
							<form action="${pageContext.request.contextPath}/mypage/product/review"
								method="post" enctype="multipart/form-data"
								class="bg-white p-6 rounded shadow-md">
								<input type="hidden" name="productId" value="${firstOption.product.productId}" />
								...
								<div class="mb-4">
									...
									<select name="score" id="score" required class="border rounded px-3 py-2 w-full">
								...
								<div class="mb-4">
									...
									<textarea name="content" id="content" rows="4" required
										class="border rounded px-3 py-2 w-full" placeholder="상품에 대한 리뷰를 작성해주세요."></textarea>
								</div>
					...
					<c:if test="${empty sessionScope.loginUser}">
						<div class="mt-8 bg-yellow-50 border border-yellow-200 rounded p-4">
							<p class="text-yellow-800">리뷰를 작성하려면 <a href="${pageContext.request.contextPath}/login" class="text-blue-600 underline">로그인</a>해주세요.</p>
						</div>
					</c:if>
				</c:if>
			</tbody>
